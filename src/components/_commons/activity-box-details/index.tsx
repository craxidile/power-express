...
const ActivityBoxDetails = (props: PropsWithChildren<ActivityBoxDetailsProps>) => {
  const {title, excerpt, tag, publishedAt, type} = props;

  return (
    <>
      <div className="mt-5 lg:mt-6 flex flex-row justify-start items-stretch">
        <div className={`py-1 px-3 ${tag === 'News' ? 'bg-green-tag' : 'bg-cta-primary'}`}>
          <span className="block text-white text-xs lg:text-sm uppercase">{tag}</span>
        </div>
      </div>
